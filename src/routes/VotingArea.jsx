...
const VotingArea = () => {
  const {state,candidates,setCandidates,setUsers,setTotalVotes,totalVotes,currentUser,users,setCurrentUser} = useStateContext()
          ...
          <div style={{display:'flex',margin:' 30px',justifyContent:'space-evenly',alignItems:'center'}}>
            ...
            {candidates?.map((candidate,key)=> (
              <div key={key} style={{ display:'flex',alignItems:'center',flexDirection:'column',margin:'15px'}}>
                ...
                <button  style={{backgroundColor:'rgb(36, 37, 46)',alignItems:'center',color:'white',padding:'15px',width:'100%',border:'none'}} onClick='' > {candidate.name} ({candidate.party})</button>
